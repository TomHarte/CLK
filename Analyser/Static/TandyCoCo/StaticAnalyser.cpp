...
Analyser::Static::TargetList Analyser::Static::TandyCoCo::GetTargets(
	...
	if(!media.disks.empty()) {
		...
		// Check first for a boot track.
		if(Storage::Disk::TandyCoCo::has_boot_track(disk)) {
			target->loading_command += L"DOS\n";
			target->rom_version = Target::ROMVersion::V11OrAbove;
		} else {
			auto loadable = [&]() -> std::optional<Storage::Disk::TandyCoCo::DirectoryEntry> {
				const auto maybe_directory = Storage::Disk::TandyCoCo::directory(disk);
