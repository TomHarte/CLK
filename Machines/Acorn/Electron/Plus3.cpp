...
void Plus3::set_control_register(const uint8_t control) {
...
void Plus3::set_control_register(const uint8_t control, const uint8_t changes) {
	if(changes&3) {
		set_drive(control&3);
	}
...
void Plus3::set_motor_on(const bool on) {
...
void Plus3::set_activity_observer(Activity::Observer *const observer) {
